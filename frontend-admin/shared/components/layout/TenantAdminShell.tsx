...
const navSections: NavSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: "Users & Roles", href: "/users", icon: Users2 },
      { label: "Compliance", href: "/compliance", icon: ShieldCheck },
    ],
  },
  {
    label: "Talent",
    items: [{ label: "Talent", href: "/talent", icon: UserCircle2 }],
  },
  {
    label: "Casting & Auditions",
    items: [{ label: "Castings", href: "/casting", icon: Megaphone }],
  },
  {
    label: "Pageants & Events",
    items: [{ label: "Events", href: "/events", icon: Crown }],
  },
  {
    label: "Influencers",
    items: [{ label: "Campaigns", href: "/influencers", icon: Sparkles }],
  },
  {
    label: "Academy",
    items: [{ label: "Academy", href: "/academy", icon: GraduationCap }],
  },
  {
    label: "Settings",
    items: [
...
export function TenantAdminShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen bg-slate-950 text-slate-200">
          ...
          <div className="flex min-w-0 items-center gap-3">
            ...
            {!collapsed && (
              <div className="min-w-0">
                ...
                <p className="truncate text-[10px] text-amber-400/90">Tenant Admin</p>
              </div>
            )}
          </div>
          <Button
      ...
      <div className={cn("flex min-h-screen flex-1 flex-col transition-[margin] duration-200", collapsed ? "ml-[72px]" : "ml-64")}>
        <header className="sticky top-0 z-20 flex h-14 shrink-0 items-center justify-between gap-4 border-b border-slate-800 bg-slate-900/80 backdrop-blur-md px-4 sm:px-6">
          <div className="flex flex-1 items-center gap-4" />
          <div className="flex items-center gap-3 border-l border-slate-700 pl-3">
            <div className="hidden text-right sm:block">
              <p className="text-sm font-medium text-white">Tenant Admin</p>
              <p className="text-xs text-amber-400/80">Demo Tenant</p>
            </div>
            <div className="flex h-9 w-9 items-center justify-center rounded-full bg-gradient-to-br from-amber-500 to-amber-600 text-sm font-semibold text-slate-900 shadow-lg shadow-amber-500/20" aria-label="User avatar">
              TA
            </div>
          </div>
        </header>
