...
import { LayoutDashboard, Megaphone, Package, FileText, BarChart3, Menu, ChevronRight, LogOut } from "lucide-react"
...
const s = ROLE_DASHBOARD_CONFIG.influencer.sidebar

const navSections = [
  { label: "Overview", items: [{ label: "Dashboard", href: BASE, icon: LayoutDashboard }] },
  { label: "Campaigns", items: [{ label: "Campaigns", href: `${BASE}/campaigns`, icon: Megaphone }] },
  { label: "Deliverables", items: [{ label: "Deliverables", href: `${BASE}/deliverables`, icon: Package }] },
  { label: "Media Kits", items: [{ label: "Media Kits", href: `${BASE}/media-kits`, icon: FileText }] },
  { label: "Reporting", items: [{ label: "Reporting", href: `${BASE}/reporting`, icon: BarChart3 }] },
]
...
export function InfluencerShell({ children }: { children: React.ReactNode }) {
  ...
  React.useEffect(() => {
    const stored = localStorage.getItem("influencerSidebarCollapsed")
    if (stored === "true") setCollapsed(true)
    setRailStyle(getSidebarRailStyle())
  }, [])

  React.useEffect(() => {
    localStorage.setItem("influencerSidebarCollapsed", String(collapsed))
  }, [collapsed])
  ...
  const page = ROLE_DASHBOARD_CONFIG.influencer.page
  if (USE_HEADER_NAV) {
    return (
      <DashboardThemeProvider role="influencer">
        <div
  ...
  return (
    <DashboardThemeProvider role="influencer">
    <div className="flex min-h-screen transition-colors" style={{ backgroundColor: page.bg, color: page.text }} data-theme="influencer">
      <aside
      ...
      >
        <div className="flex h-14 shrink-0 items-center justify-between border-b px-4" style={{ borderColor: s.border }}>
          <div className="flex min-w-0 items-center gap-3">
            <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg text-sm font-bold text-white shadow-sm" style={{ backgroundColor: s.accent }}>IN</div>
            {showFullNav && <div className="min-w-0"><p className="font-display truncate text-xs font-semibold" style={{ color: s.text }}>TalentOS</p><p className="truncate text-[10px]" style={{ color: s.accent }}>Influencer Agency</p></div>}
          </div>
      ...
      <div className={cn("flex min-h-screen flex-1 flex-col transition-[margin] duration-200", isNarrow ? "ml-[72px]" : "ml-64")}>
        <header className="sticky top-0 z-20 flex h-14 shrink-0 items-center justify-between border-b px-4 sm:px-6" style={{ backgroundColor: page.contentBg, borderColor: page.border }}>
          ...
          <div className="flex items-center gap-3 border-l pl-3" style={{ borderColor: page.border }}>
            <div className="hidden text-right sm:block"><p className="text-sm font-medium" style={{ color: page.text }}>{user?.name ?? "Influencer Agency"}</p><p className="text-xs" style={{ color: s.accent }}>Influence Studio</p></div>
            ...
            <div className="flex h-9 w-9 items-center justify-center rounded-full text-sm font-semibold text-white shadow-sm" style={{ backgroundColor: s.accent }}>IN</div>
          </div>
        </header>
