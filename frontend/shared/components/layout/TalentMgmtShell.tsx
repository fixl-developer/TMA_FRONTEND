...
import { LayoutDashboard, UserCircle2, FileSignature, Wallet, Calendar, Menu, ChevronRight, LogOut } from "lucide-react"
...
const BASE = "/talent-mgmt"
const s = ROLE_DASHBOARD_CONFIG["talent-mgmt"].sidebar
...
const navSections = [
  ...
  { label: "Contracts", items: [{ label: "Contracts", href: `${BASE}/contracts`, icon: FileSignature }] },
  { label: "Ledger", items: [{ label: "Ledger", href: `${BASE}/ledger`, icon: Wallet }] },
  { label: "Availability", items: [{ label: "Calendar", href: `${BASE}/calendar`, icon: Calendar }] },
]
const allNavItems = navSections.flatMap((section) => section.items)
const USE_HEADER_NAV = allNavItems.length <= 5

export function TalentMgmtShell({ children }: { children: React.ReactNode }) {
  ...
  React.useEffect(() => {
    const stored = localStorage.getItem("talentMgmtSidebarCollapsed")
    if (stored === "true") setCollapsed(true)
    setRailStyle(getSidebarRailStyle())
  }, [])

  React.useEffect(() => {
    localStorage.setItem("talentMgmtSidebarCollapsed", String(collapsed))
  }, [collapsed])
  ...
  const page = ROLE_DASHBOARD_CONFIG["talent-mgmt"].page
  if (USE_HEADER_NAV) {
    return (
      <DashboardThemeProvider role="talent-mgmt">
        <div
          className="flex min-h-screen flex-col transition-colors"
          style={{ backgroundColor: page.bg, color: page.text }}
          data-theme="talent-mgmt"
        >
          <header
            className="sticky top-0 z-20 border-b"
            style={{ backgroundColor: page.contentBg, borderColor: page.border }}
          >
            <div className="flex h-14 items-center justify-between px-4 sm:px-6">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  <p className="font-display truncate text-xs font-semibold" style={{ color: s.text }}>
                    TalentOS
                  </p>
                  <p className="truncate text-[10px]" style={{ color: s.accent }}>
                    Talent Management
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <Button
                  variant="ghost"
                  size="sm"
                  style={{ color: page.textMuted }}
                  className="hover:opacity-90"
                  onMouseEnter={(e) => {
                    e.currentTarget.style.backgroundColor = page.hover
                    e.currentTarget.style.color = page.text
                  }}
                  onMouseLeave={(e) => {
                    e.currentTarget.style.backgroundColor = "transparent"
                    e.currentTarget.style.color = page.textMuted
                  }}
                  onClick={() => {
                    logout()
                    router.push("/login")
            ...
            <div className="border-t px-3 py-2 sm:px-6" style={{ borderColor: page.border }}>
                      ...
                      <span
                        ...
                        style={
                          active
                            ...
                            : { borderColor: page.border, color: page.textMuted, backgroundColor: page.contentBg }
                        }
                      >
  ...
  return (
    <DashboardThemeProvider role="talent-mgmt">
    <div className="flex min-h-screen transition-colors" style={{ backgroundColor: page.bg, color: page.text }} data-theme="talent-mgmt">
      <aside
      ...
      >
        <div className="flex h-14 shrink-0 items-center justify-between border-b px-4" style={{ borderColor: s.border }}>
          <div className="flex min-w-0 items-center gap-3">
            <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg text-sm font-bold text-white shadow-sm" style={{ backgroundColor: s.accent }}>TM</div>
            {showFullNav && <div className="min-w-0"><p className="font-display truncate text-xs font-semibold" style={{ color: s.text }}>TalentOS</p><p className="truncate text-[10px]" style={{ color: s.accent }}>Talent Management</p></div>}
          </div>
      ...
      <div className={cn("flex min-h-screen flex-1 flex-col transition-[margin] duration-200", isNarrow ? "ml-[72px]" : "ml-64")}>
        <header className="sticky top-0 z-20 flex h-14 shrink-0 items-center justify-between border-b px-4 sm:px-6" style={{ backgroundColor: page.contentBg, borderColor: page.border }}>
          ...
          <div className="flex items-center gap-3 border-l pl-3" style={{ borderColor: page.border }}>
            <div className="hidden text-right sm:block"><p className="text-sm font-medium" style={{ color: page.text }}>{user?.name ?? "Talent Management"}</p><p className="text-xs" style={{ color: s.accent }}>Elite Talent Co</p></div>
            ...
            <div className="flex h-9 w-9 items-center justify-center rounded-full text-sm font-semibold text-white shadow-sm" style={{ backgroundColor: s.accent }}>TM</div>
          </div>
        </header>
