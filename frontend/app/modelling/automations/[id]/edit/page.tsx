...
import { getAutomationById, saveAutomation } from "@/shared/services/modellingAutomationService"
...
export default function EditAutomationPage() {
  ...
  useEffect(() => {
    if (!id) return
    getAutomationById(id, tenantId).then((a) => {
      if (a) {
        setName(a.name)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    showToast("Automation updated (mock)", "success")
    router.push(`/modelling/automations/${id}`)
    setSaving(false)
  }
    ...
    <AgenciesPage>
      ...
      <div className="mt-6">
        <Button asChild variant="ghost" size="sm" className="mb-4 text-[#57534E] hover:text-[#1C1917]">
          <Link href={`/modelling/automations/${id}`} className="flex items-center gap-2">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex gap-2 pt-2">
                ...
                <Button type="button" variant="outline" asChild>
                  <Link href={`/modelling/automations/${id}`}>Cancel</Link>
                </Button>
              </div>
            </form>
          </CardContent>
