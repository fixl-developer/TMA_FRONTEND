...
const navSections: NavSection[] = [
  ...
  { label: "Process", items: [{ label: "Process Builder", href: "/process", icon: Workflow }] },
  { label: "Registration", items: [{ label: "Registration", href: "/registration", icon: UserPlus }] },
  { label: "Judges", items: [{ label: "Judges", href: "/judges", icon: Award }] },
  { label: "Sponsors", items: [{ label: "Sponsors", href: "/sponsors", icon: BadgeDollarSign }] },
  { label: "Results", items: [{ label: "Results", href: "/results", icon: Trophy }] },
]
      ...
      >
        <div className="flex h-14 shrink-0 items-center justify-between border-b border-slate-800 px-4">
          <div className="flex min-w-0 items-center gap-3">
            <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-gradient-to-br from-violet-500 to-violet-600 text-sm font-bold text-white shadow-lg shadow-violet-500/20">
              PO
            </div>
            {!collapsed && (
              <div className="min-w-0">
                <p className="font-display truncate text-xs font-semibold tracking-wide text-white">TalentOS</p>
                <p className="truncate text-[10px] text-violet-400/90">Pageant Organizer</p>
              </div>
            )}
          </div>
                        ...
                        <li key={item.href}>
                          <Link
                            ...
                            className={cn(
                              ...
                              active ? "bg-violet-500/15 font-medium text-violet-400" : "text-slate-400 hover:bg-slate-800/80 hover:text-white"
                            )}
                          >
                            <span
                              className={cn(
                                "flex h-8 w-8 shrink-0 items-center justify-center rounded-lg",
                                active ? "bg-violet-500/30 text-violet-400" : "bg-slate-800 text-slate-500 group-hover:bg-slate-700 group-hover:text-slate-300"
                              )}
                            >
        ...
        <header className="sticky top-0 z-20 flex h-14 shrink-0 items-center justify-between border-b border-slate-800 bg-slate-900/80 backdrop-blur-md px-4 sm:px-6">
          ...
          <div className="flex items-center gap-3 border-l border-slate-700 pl-3">
            <div className="hidden text-right sm:block">
              <p className="text-sm font-medium text-white">Pageant Organizer</p>
              <p className="text-xs text-violet-400/80">Miss India 2025</p>
            </div>
            <div className="flex h-9 w-9 items-center justify-center rounded-full bg-gradient-to-br from-violet-500 to-violet-600 text-sm font-semibold text-white shadow-lg shadow-violet-500/20">
              PO
            </div>
          </div>
        </header>
